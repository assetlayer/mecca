...
async function main() {
  ...
  // Check if pool exists using the correct extsload function
  console.log("\n=== Checking Pool Existence ===");
  
  try {
    // Use the single bytes32 version of extsload
    const slot0 = await pm["extsload(bytes32)"](poolId);
    console.log("✓ Pool exists (extsload):", slot0);
    
    // If pool exists, try to add liquidity
    ...
    try {
      ...
      try {
        ...
        // Try to add liquidity now
        console.log("\n=== Attempting to Add Liquidity After Initialization ===");
        ...
        console.log("✗ Pool still does not exist after initialization:", e?.error?.message || e?.message || e);
      }

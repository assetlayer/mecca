...
async function main() {
  ...
  console.log("=== Testing Small V3 Pool Swap ===");
  ...
  // Try a very small swap (0.1 token0 for token1)
  const swapAmount = ethers.parseUnits("0.1", decimals0);
  ...
  const expectedOutput = (swapAmount * reserve1) / (reserve0 + swapAmount);
  console.log("Expected output:", ethers.formatUnits(expectedOutput, decimals1));

  // Perform swap
  const swapTx = await pool.swap(0, expectedOutput, await signer.getAddress());
  await swapTx.wait();

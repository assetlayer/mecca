...
async function main() {
  ...
  // Check if any of the pools we tried to create actually exist
  const poolConfigs = [
  ...
  for (const config of poolConfigs) {
    ...
    try {
      ...
      console.log(`✓ Pool exists: fee=${config.fee}, tickSpacing=${config.tickSpacing}`);
      ...
      console.log(`✗ Pool does not exist: fee=${config.fee}, tickSpacing=${config.tickSpacing}`);
    }
  }
